...
(() => {
  ...
  const documentRoot = new URL(`${document.body.dataset.docRoot || "."}/`, document.baseURI);
  ...
  function renderSearch() {
    ...
    if (query) {
      ...
      matches = index.filter(entry => entry.kind !== "멤버").slice(0, 6);
    }
    status.textContent = query ? `${matches.length}개 결과${matches.length > 60 ? " · 상위 60개 표시" : ""}` : "API 둘러보기";
    ...
    for (const entry of matches.slice(0, 60)) {
      ...
      link.href = new URL(entry.url, documentRoot).href;
  ...
  dialog.addEventListener("close", () => {
    // Closing after an in-page result must preserve the destination's scroll.
    if (searchOpener?.getClientRects().length) searchOpener.focus({ preventScroll: true });
