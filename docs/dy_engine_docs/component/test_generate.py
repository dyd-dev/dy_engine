...
Run with: python docs/rhi/component/test_generate.py
...
class ReadingContent(HTMLParser):
    """Inspect visible API articles without treating sidebar labels as content."""
    ...
    def handle_starttag(self, tag, attrs):
        attrs = dict(attrs)
        if tag == "article" and "reference-api" in attrs.get("class", "").split():
            self.articles[attrs["id"]] = {"parents": list(self.stack), "attrs": attrs, "text": ""}
    ...
    def handle_data(self, data):
        for tag, attrs in self.stack:
            if tag == "article" and attrs.get("id") in self.articles:
                self.articles[attrs["id"]]["text"] += data
...
class GenerateTests(unittest.TestCase):
    ...
    def test_public_and_internal_definitions_keep_separate_pages_and_signatures(self):
        ...
        self.assertEqual(len({reference.reading_url(key) for key, _ in devices}), 2)
        articles = ReadingContent(pages["index.html"]).articles
        for key, device in devices:
            ...
            article = articles[urlsplit(reference.reading_url(key)).fragment]
            self.assertIn(own_signature, article["text"])
            self.assertNotIn(other_signature, article["text"])
        self.assert_local_links_resolve(pages)
    ...
    def test_unexpanded_declarations_keep_diagnostics_and_source_text(self):
        ...
        self.assertIn("DECLARE_WIDGET(HiddenWidget)", html.unescape(pages["index.html"]))
    ...
    def test_index_exposes_every_api_as_an_expanded_reading_article(self):
        expected = {}
        for header, prefix, count in (("Texture.h", "Texture", 7), ("nested/Buffer.h", "Buffer", 2)):
            declarations = []
            for number in range(count):
                name = f"{prefix}{number}"
                comment = f"{name}의 전체 상세 설명입니다."
        ...
        reference, pages = self.build()
        content = ReadingContent(pages["index.html"])
        self.assertEqual(len(content.articles), len(expected))
        groups = PageLinks(pages["index.html"]).nav_groups
        self.assertEqual({group["data-group"] for group in groups}, set(reference.sources))
        ...
        for key, declaration in reference.symbols.items():
            with self.subTest(api=declaration.name):
                target = urlsplit(reference.reading_url(key))
                self.assertEqual(target.path, "index.html")
                article = content.articles[target.fragment]
                parent_tags = [tag for tag, _ in article["parents"]]
                self.assertIn("main", parent_tags)
                self.assertNotIn("details", parent_tags)
                self.assertNotIn("hidden", article["attrs"])
                self.assertFalse(any("hidden" in attrs for _, attrs in article["parents"]))
                for expected_text in expected[declaration.name]:
                    self.assertIn(expected_text, article["text"])
                self.assertIn("bool Run(int count);", article["text"])
                self.assertIn(reference.reading_url(key), content.nav_urls)
                opened_groups = [group["data-group"] for group in PageLinks(pages[reference.url(key)]).nav_groups if "open" in group]
                self.assertEqual(opened_groups, [declaration.header])

        for header in reference.sources:
            self.assertIn("index.html#" + reference.header_anchor(header), PageLinks(pages["index.html"]).hrefs)
        assignment = re.search(r"=\s*(\[[\s\S]*\])\s*;?\s*$", pages["component/assets/search-index.js"])
        for entry in json.loads(assignment.group(1)):
            self.assertEqual(urlsplit(entry["url"]).path, "index.html")
        self.assert_local_links_resolve(pages)
