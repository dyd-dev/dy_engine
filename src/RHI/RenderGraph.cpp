...
namespace dy::RHI
{
	...
	RenderGraphPass& RenderGraphPass::SetPipeline(RHI::PipelineHandle pipeline)
	{
		m_pipeline = pipeline;
		++m_revision;
		return *this;
	...
	RGResourceHandle RenderGraph::ImportTexture(const std::string& name, RHI::TextureHandle texture)
	{
	...
	RGResourceHandle RenderGraph::ImportBuffer(const std::string& name, RHI::BufferHandle buffer)
	{
		auto it = m_resourceNameToHandle.find(name);
		if (it != m_resourceNameToHandle.end())
		{
