...
namespace dyf::RHI { class ICommandList; }
...
namespace dyf::RHI
{
	...
	struct RGResourceDesc
	{
		...
		RHI::TextureHandle texturePtr = nullptr;
		RHI::BufferHandle bufferPtr = nullptr;
	};
	...
	public:
		...
		RenderGraphPass& SetPipeline(RHI::PipelineHandle pipeline);
		RenderGraphPass& SetExecute(std::function<void(RHI::ICommandList*)> callback);

		[[nodiscard]] const std::string& GetName() const { return m_name; }
		[[nodiscard]] uint32_t GetIndex() const { return m_index; }
		[[nodiscard]] uint64_t GetRevision() const { return m_revision; }
		[[nodiscard]] RHI::PipelineHandle GetPipeline() const { return m_pipeline; }
	...
	private:
		...
		std::function<void(RHI::ICommandList*)> m_executeCallback;
	};
	...
	public:
		...
		RGResourceHandle ImportTexture(const std::string& name, RHI::TextureHandle texture);
		RGResourceHandle ImportBuffer(const std::string& name, RHI::BufferHandle buffer);
