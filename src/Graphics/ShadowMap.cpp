...
namespace dy::Graphics
{
	namespace
	{
		float ShadowDot(const Math::float3& a, const Math::float3& b)
		{
			return a.x * b.x + a.y * b.y + a.z * b.z;
		}

		Math::float3 ShadowCross(const Math::float3& a, const Math::float3& b)
		{
			return Math::float3(
				a.y * b.z - a.z * b.y,
				a.z * b.x - a.x * b.z,
		...
		Math::float3 ShadowNormalize(const Math::float3& v)
		{
			const float len = std::sqrt(ShadowDot(v, v));
		...
		float ShadowLength(const Math::float3& value)
		{
			return std::sqrt(ShadowDot(value, value));
		}
		...
		{
			const Math::float3 forward = ShadowNormalize(Subtract(target, eye));
			const Math::float3 right   = ShadowNormalize(ShadowCross(forward, up));
			const Math::float3 camUp   = ShadowCross(right, forward);

			Math::float4x4 view = Math::float4x4::Identity();
			view.m[0]  = right.x;
			view.m[4]  = right.y;
			view.m[8]  = right.z;
			view.m[12] = -ShadowDot(right, eye);
			view.m[1]  = camUp.x;
			view.m[5]  = camUp.y;
			view.m[9]  = camUp.z;
			view.m[13] = -ShadowDot(camUp, eye);
			view.m[2]  = -forward.x;
			view.m[6]  = -forward.y;
			view.m[10] = -forward.z;
			view.m[14] = ShadowDot(forward, eye);
			return view;
		}
		...
		Math::float4x4 ShadowMultiplyColumnMajor(const Math::float4x4& lhs, const Math::float4x4& rhs)
		{
	...
	{
		const Math::float3 lightForward = ShadowNormalize(lightDirection);
		...
		return ShadowMultiplyColumnMajor(proj, view);
	}
		...
		float padding)
	{
		...
		const float radius = std::max(ShadowLength(halfExtent), 0.1f);
