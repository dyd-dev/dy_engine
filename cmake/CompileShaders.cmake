...
function(dy_compile_shader target name stage)
    ...
    if(NOT stage MATCHES "^(vert|hull|domain|frag|comp)$")
        message(FATAL_ERROR "dy_compile_shader: stage must be vert, hull, domain, frag or comp")
    endif()
    ...
    if(USE_VULKAN)
        ...
        set(glsl_stage "${stage}")
        if(stage STREQUAL "hull")
            set(glsl_stage tesc)
        ...
        add_custom_command(OUTPUT "${binary}"
            ...
            COMMAND "${DY_GLSLC}" "-fshader-stage=${glsl_stage}" ${includes} "${SHADER_GLSL}" -o "${binary}"
