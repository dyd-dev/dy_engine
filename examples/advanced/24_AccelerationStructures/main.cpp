...
namespace
{
    ...
    {
        std::cout <<
        "가속 구조: 현재는 지원 조회와 미지원 단계 안내를 제공하는 예제입니다.\n"
        "Feature::RayQuery/RayTracingPipeline 조회는 관련 기능 상태만 제공합니다. 가속 구조 생성 가능 여부를 별도로 조회하는 공개 항목은 없습니다.\n"
        "공개 RHI에는 BLAS/TLAS 핸들·geometry/instance descriptor·빌드/업데이트 명령·scratch 요구량 조회가 없습니다.\n"
        "가속 구조의 저장/빌드 접근 상태와 resource binding도 없어 삼각형 BLAS 또는 인스턴스 TLAS를 실제 생성하지 않습니다.\n"
...
int main(int argc, char** argv)
{
    ...
    {
        std::cout << "사용법: AdvancedAccelerationStructures [--help]\n"
    ...
    // 완성 경로: geometry/instance 입력 정의 → 빌드 크기/scratch 조회 → BLAS 빌드 → TLAS 빌드 → 동기화/수명 검증.
