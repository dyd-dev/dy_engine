...
int main(int argc, char** argv)
{
    try
    {
        ...
        for(int i = 1; i < argc; ++i)
        {
            ...
            if(argument == "--help")
            {
                std::puts("AdvancedMultithreadRecording --mode serial|parallel --workers N --frames N --capture image.ppm --validation\n"
                    ...
                    "RHI and native command recording run on workers; submission preserves graph order.");
                return 0;
            }
        ...
        std::puts("CPU recording includes native translation; submission is measured separately. These are not GPU timings.");
        return 0;
    }
