...
if(value=="clustered") params.sizeTime[3]=0;
else if(value=="all") params.sizeTime[3]=1;
else if(value=="count") params.sizeTime[3]=2;
else throw std::runtime_error("Unknown mode: "+value);
}else if(arg=="--lights") { double n=Number(value,1,64);if(std::floor(n)!=n) throw std::runtime_error("This option requires an integer.");params.extra[2]=float(n);}
...
auto clusterShader=resources.Keep(device.CreateShader({ShaderStage::Compute,ShaderData::clusterEntryPoint,ShaderData::cluster,ShaderData::clusterSize}));
auto lightingShader=resources.Keep(device.CreateShader({ShaderStage::Fragment,ShaderData::lightingEntryPoint,ShaderData::lighting,ShaderData::lightingSize}));
    ...
    // 16*16*8 cells, each containing count + up to 64 light indices; uint32 stride.
    constexpr uint32_t clusterCount=16*16*8,wordsPerCluster=65;
    static_assert(sizeof(uint32_t)==4);
    BufferDesc listDesc;listDesc.size=clusterCount*wordsPerCluster*sizeof(uint32_t);listDesc.stride=sizeof(uint32_t);
    listDesc.usage=BufferUsage::Storage;listDesc.initialState=ResourceState::Undefined;
    ...
    while(!frameLimit || frame<frameLimit) {
        ...
        // GPU builds light lists; one invocation per world-space cluster, no CPU culling.
        const ResourceBarrierDesc writable{lightLists,nullptr,listsInitialized?ResourceState::ShaderResource:ResourceState::Undefined,ResourceState::UnorderedAccess,{}};
        commands->ResourceBarrier(&writable,1);commands->BindComputePipeline(clusterPipeline);
        commands->BindResourceSet(clusterSet);commands->SetInlineConstants(0,sizeof(params),&params);
        commands->Dispatch(clusterCount/64,1,1);
        const ResourceBarrierDesc readable{lightLists,nullptr,ResourceState::UnorderedAccess,ResourceState::ShaderResource,{}};
        commands->ResourceBarrier(&readable,1);
        // Fragment shader consumes the GPU-generated cluster list.
        {
