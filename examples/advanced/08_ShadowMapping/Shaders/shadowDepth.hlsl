...
struct Output { float depth :SV_Depth; };
...
if(p.settings.w==1.0) {
    float cascade=floor(uv.x*2.0);float2 local=float2(frac(uv.x*2.0),uv.y);
    float extent=cascade<0.5 ? 12.0 : 28.0;
    origin=float3((local-0.5)*extent+float2(0.0,1.0),6.0);
}
if(p.settings.w==2.0) {
    float2 tile=floor(uv*float2(3.0,2.0)),q=frac(uv*float2(3.0,2.0))*2.0-1.0;
