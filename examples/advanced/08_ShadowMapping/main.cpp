...
int main(int argc,char** argv) try {
    Params params;
    params.settings[1]=0.0015f;params.settings[2]=1;params.settings[3]=0;params.extra[0]=8;params.extra[1]=4;
    ...
    for(int i=1;i<argc;i++) {
        ...
        if(arg=="--help") {
            std::puts("ShadowMapping\n--shadow-type directional|cascaded|point, --mode shadow|unshadowed|depth|visibility|regions\n--bias [0,0.1], --pcf [0,2] (integer), --split [4,16], --light-height [2,8], --exposure [0.01,20]\nCascades use two manually fitted camera-distance intervals; point atlas stores radial depth.\n--frames N (0=unlimited), --capture path.ppm, --time SECONDS, --validation\n--capture defaults to 1 frames unless --frames is supplied.");
            return 0;
        }
...
if(value=="shadow") params.sizeTime[3]=0;
else if(value=="unshadowed") params.sizeTime[3]=1;
else if(value=="depth") params.sizeTime[3]=2;
else if(value=="visibility") params.sizeTime[3]=3;
else if(value=="regions") params.sizeTime[3]=4;
else throw std::runtime_error("Unknown mode: "+value);
}else if(arg=="--shadow-type") {if(value=="directional") params.settings[3]=0;else if(value=="cascaded") params.settings[3]=1;else if(value=="point") params.settings[3]=2;else throw std::runtime_error("Unknown shadow type.");}
else if(arg=="--bias") { double n=Number(value,0,0.1);params.settings[1]=float(n);}
else if(arg=="--pcf") { double n=Number(value,0,2);if(std::floor(n)!=n) throw std::runtime_error("This option requires an integer.");params.settings[2]=float(n);}
else if(arg=="--split") { double n=Number(value,4,16);params.extra[0]=float(n);}
else if(arg=="--light-height") { double n=Number(value,2,8);params.extra[1]=float(n);}
    ...
    dyf::Platform::Window window(960,640,"Advanced / ShadowMapping");
...
auto shadowDepthShader=resources.Keep(device.CreateShader({ShaderStage::Fragment,ShaderData::shadowDepthEntryPoint,ShaderData::shadowDepth,ShaderData::shadowDepthSize}));
auto lightingShader=resources.Keep(device.CreateShader({ShaderStage::Fragment,ShaderData::lightingEntryPoint,ShaderData::lighting,ShaderData::lightingSize}));
    ...
    sampler.mipLodBias=sampler.minLod=sampler.maxLod=0;
        // Pipeline: shadow
    std::vector<ResourceBindingLayout> shadowBindings;
    std::array<ColorAttachmentDesc,0> shadowColors{};
    for(auto& attachment:shadowColors) attachment={Format::B8G8R8A8_UNORM,{},ColorWriteMask::All};
    GraphicsPipelineDesc shadowDesc;shadowDesc.vertexShader=vertexShader;shadowDesc.fragmentShader=shadowDepthShader;
    shadowDesc.topology=PrimitiveTopology::TriangleList;shadowDesc.raster={FillMode::Solid,CullMode::None,FrontFace::CounterClockwise,0,0,0};
    shadowDesc.colorAttachments=shadowColors.data();shadowDesc.colorAttachmentCount=uint32_t(shadowColors.size());
    shadowDesc.depthStencil.format=Format::D32_FLOAT;shadowDesc.depthStencil.depthWriteEnabled=true;shadowDesc.depthStencil.depthTestEnabled=true;shadowDesc.depthStencil.depthCompareOp=CompareOp::Always;
    shadowDesc.layout={shadowBindings.data(),uint32_t(shadowBindings.size()),sizeof(Params),ShaderStageFlags::Fragment,15};
    ...
    TextureHandle shadowMap=nullptr;
ResourceSetHandle lightingSet=nullptr;
    ...
    while(!frameLimit || frame<frameLimit) {
        ...
        if(dimensions.width!=imageWidth || dimensions.height!=imageHeight) {
            ...
            imageWidth=dimensions.width;imageHeight=dimensions.height;initializeImages=true;
                        TextureDesc shadowMapDesc;shadowMapDesc.width=1536;shadowMapDesc.height=1024;
            shadowMapDesc.format=Format::D32_FLOAT;shadowMapDesc.usage=TextureUsage::DepthStencil|TextureUsage::ShaderResource;
            shadowMap=imageResources->Keep(device.CreateTexture(shadowMapDesc));

                        std::vector<ResourceBinding> lightingSetBindings;
            lightingSetBindings.push_back({0,0,nullptr,shadowMap,0,0,{}});
        ...
        auto* commands=frameResources.Keep(device.AcquireCommandList());
                if(initializeImages) {
            { const ResourceBarrierDesc barrier{nullptr,shadowMap,ResourceState::Undefined,ResourceState::DepthWrite,{}};commands->ResourceBarrier(&barrier,1);
              DepthStencilAttachment a;a.texture=shadowMap;a.state=ResourceState::DepthWrite;a.depthLoadOp=LoadOp::Clear;a.depthStoreOp=StoreOp::Store;a.clearDepth=1;commands->BeginRendering({nullptr,0,&a});
              commands->EndRendering();const ResourceBarrierDesc ready{nullptr,shadowMap,ResourceState::DepthWrite,ResourceState::ShaderResource,{}};commands->ResourceBarrier(&ready,1); }
        }

                // 1. D32 atlas: one directional map, two cascade tiles, or six point-light faces.
        {
            const ResourceBarrierDesc shadowMapBarrier{nullptr,shadowMap,ResourceState::ShaderResource,ResourceState::DepthWrite,{}};commands->ResourceBarrier(&shadowMapBarrier,1);
            DepthStencilAttachment depth;depth.texture=shadowMap;depth.state=ResourceState::DepthWrite;depth.depthLoadOp=LoadOp::Clear;depth.depthStoreOp=StoreOp::Store;depth.clearDepth=1;
            commands->BeginRendering({nullptr,0,&depth});
            commands->BindGraphicsPipeline(shadowPipeline);
            commands->SetViewport({0,0,float(1536),float(1024),0,1});commands->SetScissor({0,0,1536,1024});
            commands->SetInlineConstants(0,sizeof(params),&params);commands->DrawInstanced(3,1,0,0);commands->EndRendering();
            const ResourceBarrierDesc shadowMapReady{nullptr,shadowMap,ResourceState::DepthWrite,ResourceState::ShaderResource,{}};commands->ResourceBarrier(&shadowMapReady,1);
        }
        // 2. Select cascade/cube face and compare receiver depth with tile-clamped PCF.
        {
    ...
    std::fprintf(stderr,"ShadowMapping: %s\n",error.what());return 1;
}
