...
namespace
{
    ...
    {
        std::cout <<
        "레이 쿼리 그림자: 현재는 지원 조회와 미지원 단계 안내를 제공하는 예제입니다.\n"
        "레이 쿼리는 shader 내부 광선 질의이므로 host의 전용 trace 명령만 추가해서 완성되는 기능이 아닙니다.\n"
        "현재 RHI에는 scene BLAS/TLAS 생성·빌드와 가속 구조 resource binding이 없어 shader에 질의 대상을 전달할 수 없습니다.\n"
        "RayQuery를 사용하는 shader 및 필요한 백엔드 기능 활성화/검증 경로도 준비되어 있지 않아 실제 레이 쿼리 그림자는 렌더링하지 않습니다.\n"
...
int main(int argc, char** argv)
{
    ...
    {
        std::cout << "사용법: AdvancedRayQueryShadows [--help]\n"
    ...
    // 완성 경로: scene BLAS/TLAS 빌드 → 가속 구조 shader binding → 광원 가림 질의 → 그림자 합성/검증.
