...
struct Options {
    ...
    float nearPlane=.1f, farPlane=100.f;
    std::string mode="default", capture;
};
Options Parse(int argc,char** argv) {
    ...
    if(options.flight<1 || options.flight>4 ||
        !std::isfinite(options.nearPlane) || !std::isfinite(options.farPlane) || options.nearPlane<=0 || options.farPlane<=options.nearPlane)
...
int main(int argc,char** argv) try {
    ...
    Check(options.mode=="default" || options.mode=="split" || options.mode=="raw" || options.mode=="linear","Invalid mode");
    ...
    producerDesc.colorAttachments=nullptr;producerDesc.colorAttachmentCount=0;
    producerDesc.depthStencil.format=Format::D32_FLOAT;producerDesc.depthStencil.depthWriteEnabled=true;
    ...
    while(!options.frames || frame<options.frames){
        ...
        if(!intermediate || intermediate->GetDesc().width!=extent.width || intermediate->GetDesc().height!=extent.height){
            ...
            texture.format=Format::D32_FLOAT;texture.usage=TextureUsage::DepthStencil|TextureUsage::ShaderResource;
        ...
        const float nearPlane=options.nearPlane,farPlane=options.farPlane;
        ...
        for(uint32_t pass=0;pass<1;++pass){
            DepthStencilAttachment output;output.texture=intermediate;output.state=ResourceState::DepthWrite;
            output.depthLoadOp=LoadOp::Clear;output.depthStoreOp=StoreOp::Store;output.clearDepth=1;
        ...
        const ResourceBarrierDesc ready[]={{nullptr,intermediate,ResourceState::DepthWrite,ResourceState::ShaderResource,{}},
