...
int main()
{
    ...
    try {
        ...
        dy::Platform::Window window(windowWidth, windowHeight, "RHI / MeshMaterial");
        ...
        const auto mesh = CreateCubeMesh(1);
        ...
        if (!dy::Graphics::LoadImage(DY_EXAMPLE_IMAGE, image)) throw std::runtime_error("Image loading failed.");
        TextureDesc baseDesc;
        baseDesc.width = image.width; baseDesc.height = image.height;
        baseDesc.format = Format::R8G8B8A8_UNORM;
        baseDesc.usage = TextureUsage::ShaderResource;
        ...
        ResourceBarrierDesc baseReady{nullptr, base, ResourceState::Undefined, ResourceState::CopyDestination, {}};
        upload->ResourceBarrier(&baseReady, 1);
        if (!device.UpdateTexture(*upload, base, 0, 0, image.rgba8.data(), static_cast<uint32_t>(baseBytes),
            baseDesc.width * 4, static_cast<uint32_t>(baseBytes))) throw std::runtime_error("Texture upload failed.");
        baseReady.before = ResourceState::CopyDestination; baseReady.after = ResourceState::ShaderResource;
        upload->ResourceBarrier(&baseReady, 1);
        ...
        draws[0].textureFlags = 1;
        draws[0].materialParams.y = 0.8f;
        draws[0].modelMatrix = Math::Translation({-0.7f, 0, 0}) * Math::RotationZ(0.35f);
        draws[1].baseColor = {0.9f, 0.45f, 0.15f, 1};
        draws[1].materialParams.x = 0.85f;
        draws[1].materialParams.y = 0.2f;
        draws[1].modelMatrix = Math::Translation({0.7f, 0, 0}) * Math::RotationZ(-0.35f);
        upload->Close();
