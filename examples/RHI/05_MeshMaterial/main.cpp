...
int main()
{
	...
	dyf::Platform::Window window(width, height, "RHI / MeshMaterial");
	...
	sampler.addressU = sampler.addressV = sampler.addressW = dyf::RHI::SamplerAddressMode::Repeat;
	sampler.minLod = sampler.maxLod = sampler.mipLodBias = 0;
	const std::array<dyf::RHI::ResourceBindingLayout, 3> bindingLayout = {{
	...
	dyf::Image image;
	if (!dyf::LoadImage(DY_EXAMPLE_IMAGE, image))
	{
		std::fprintf(stderr, "Image loading failed.\n");
		return 1;
	}
	dyf::RHI::TextureDesc baseDesc;
	baseDesc.width = image.GetWidth();
	baseDesc.height = image.GetHeight();
	baseDesc.format = image.GetColorSpace() == dyf::ColorSpace::Srgb
		? dyf::RHI::Format::R8G8B8A8_UNORM_SRGB : dyf::RHI::Format::R8G8B8A8_UNORM;
	baseDesc.usage = dyf::RHI::TextureUsage::ShaderResource;
	const uint64_t baseBytes = static_cast<uint64_t>(baseDesc.width) * baseDesc.height * 4;
	if (!baseDesc.width || !baseDesc.height || baseBytes > UINT32_MAX)
	{
		std::fprintf(stderr, "Texture data exceeds the RHI upload range.\n");
		return 1;
	}
	auto* base = resources.Keep(device.CreateTexture(baseDesc));
	if (!base)
	{
		std::fprintf(stderr, "Resource creation failed.\n");
		return 1;
	}
	dyf::RHI::ResourceBarrierDesc baseReady{nullptr, base,
		dyf::RHI::ResourceState::Undefined, dyf::RHI::ResourceState::CopyDestination, {}};
	upload->ResourceBarrier(&baseReady, 1);
	if (!device.UpdateTexture(*upload, base, 0, 0, image.GetPixels().data(), static_cast<uint32_t>(baseBytes),
		baseDesc.width * 4, static_cast<uint32_t>(baseBytes)))
	{
		std::fprintf(stderr, "Texture upload failed.\n");
		return 1;
	}
	baseReady.before = dyf::RHI::ResourceState::CopyDestination;
	baseReady.after = dyf::RHI::ResourceState::ShaderResource;
	upload->ResourceBarrier(&baseReady, 1);

	std::array<DrawConstants, 2> draws{};
	for (auto& draw : draws)
	{
	...
	draws[0].useBaseColorTexture = 1;
	draws[0].roughness = 0.8f;
	draws[0].modelMatrix = dyf::Math::Translation({-0.7f, 0, 0}) * dyf::Math::RotationZ(0.35f);
	draws[1].baseColor = {0.9f, 0.45f, 0.15f, 1};
	draws[1].metallic = 0.85f;
	draws[1].roughness = 0.2f;
	draws[1].modelMatrix = dyf::Math::Translation({0.7f, 0, 0}) * dyf::Math::RotationZ(-0.35f);
	upload->Close();
	...
	const std::array<dyf::RHI::ResourceBinding, 2> materialBindings = {{
		{0, 0, nullptr, base, 0, 0, {}},
