...
int main()
{
    ...
    try {
        ...
        dy::Platform::Window window(windowWidth, windowHeight, "RHI / Scene");
        ...
        while (window.IsRunning()) {
            ...
            draws[0].modelMatrix=parent*Math::Translation({-0.8f,0,0})*Math::RotationX(0.4f);
            draws[1].modelMatrix=parent*Math::Translation({0.6f,0,0.2f})*Math::Scaling(0.6f);
            draws[2].modelMatrix=parent*Math::Translation({0.6f,0,0.85f})*Math::Scaling(0.3f);
