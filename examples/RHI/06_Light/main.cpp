...
int main()
{
    ...
    try {
        ...
        dy::Platform::Window window(windowWidth, windowHeight, "RHI / Light");
        ...
        lighting.directionalLightColor.w=0.3f;
        lighting.directionalLights[0].directionIntensity.w=0.3f;
        ...
        const auto spotDirection=Math::NormalizeOr({-0.6f,0.3f,-1},{0,0,-1});
        ...
        for(auto& draw:draws) draw.materialParams.y=0.65f;
        draws[0].modelMatrix=Math::Translation({-0.7f,0,0});
        draws[1].modelMatrix=Math::Translation({0.7f,0,0});
        draws[2].modelMatrix=Math::Translation({0,0,-0.65f})*Math::Scaling(Math::float3{3,3,0.1f});
        auto* base = white;
        upload->Close();
        ...
        const auto view = Math::LookAtRH({2.6f,-4,2.7f}, {0,0,0}, {0,0,1});
