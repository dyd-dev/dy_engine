...
vec3 EvaluateDirectLight(vec3 normal, vec3 viewDir, vec3 lightDir, vec3 incidentIlluminance,
    vec3 albedo, float metallic, float roughness, float visibility) {
    float ndotl = max(dot(normal, lightDir), 0.0);
    float ndotv = max(dot(normal, viewDir), 0.0);
    if (ndotl <= 0.0 || ndotv <= 0.0 || visibility <= 0.0) return vec3(0.0);
...
vec3 EvaluateAreaSample(vec3 normal, vec3 viewDir, vec3 samplePosition, vec3 emitterDirection,
    vec3 emittedRadiance, float sampleArea, vec3 albedo, float metallic, float roughness) {
    vec3 toLight = samplePosition - fragWorldPosition;
    float distanceSquared = dot(toLight, toLight);
    if (distanceSquared <= 0.000001 || sampleArea <= 0.0) return vec3(0.0);
    vec3 lightDir = toLight * inversesqrt(distanceSquared);
    vec3 incidentIlluminance = emittedRadiance * max(dot(emitterDirection, -lightDir), 0.0) * sampleArea / distanceSquared;
    return EvaluateDirectLight(normal, viewDir, lightDir, incidentIlluminance, albedo, metallic, roughness, 1.0);
}

vec3 EvaluateRectAreaLight(RendererRectAreaLight light, vec3 normal, vec3 viewDir,
...
vec3 EvaluateDiscAreaLight(RendererDiscAreaLight light, vec3 normal, vec3 viewDir,
    vec3 albedo, float metallic, float roughness) {
...
int ResolvePointShadowFace(vec3 fromLight) {
    ...
    if (absoluteDirection.x >= absoluteDirection.y && absoluteDirection.x >= absoluteDirection.z) return fromLight.x >= 0.0 ? 0 : 1;
    if (absoluteDirection.y >= absoluteDirection.x && absoluteDirection.y >= absoluteDirection.z) return fromLight.y >= 0.0 ? 2 : 3;
...
void main() {
    ...
    for (int lightIndex = 0; lightIndex < directionalCount; ++lightIndex) {
        ...
        float visibility = shadowType == 1 && shadowIndex == lightIndex
            ? CalculateShadowVisibility(fragWorldPosition, normal, lightDir) : 1.0;
