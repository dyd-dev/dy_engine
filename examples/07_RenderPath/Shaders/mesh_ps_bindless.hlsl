// mesh_ps_bindless.hlsl - D3D12 bindless 변형.
// 머티리얼 텍스처를 단일 SRV 가 아니라 글로벌 디스크립터 힙을 덮는 unbounded SRV 배열에서
// per-draw 디스크립터 인덱스로 샘플한다. 인덱스는 b0(textureIndices, emissiveTextureIndex)로 전달.
// 루트 시그니처의 param 1(register t0, space0, unbounded descriptor table)을 그대로 사용한다.
...
cbuffer DrawConstants : register(b0)
{
    ...
    float4 textureIndices; // x=baseColor, y=metallicRoughness, z=normal, w=occlusion
};
...
float4 SampleBindless(float index, float2 uv)
{
    return BindlessTextures[(uint)(index + 0.5)].Sample(LinearSampler, uv);
}
...
float3 GetNormal(PSInput input, uint textureFlags)
{
    ...
    float3 tangentNormal = SampleBindless(textureIndices.z, input.uv).xyz * 2.0 - 1.0;
    tangentNormal.xy *= normalScale;
    return normalize(mul(tangentNormal, tbn));
}

float CalculateShadowVisibility(float4 lightSpacePosition, float3 normal, float3 lightDir, uint textureFlags)
{
    if (directionalLightDirection.w < 0.5)
    {
        return 1.0;
    }
    if ((textureFlags & DY_TEXTURE_FLAG_RECEIVE_SHADOW) == 0u)
    {
        return 1.0;
    }
...
float4 main(PSInput input) : SV_TARGET
{
    ...
    {
        albedo *= SampleBindless(textureIndices.x, input.uv).rgb;
    }
    ...
    {
        float4 metallicRoughness = SampleBindless(textureIndices.y, input.uv);
    ...
    {
        occlusion *= SampleBindless(textureIndices.w, input.uv).r;
    }
    ...
    float ndotl = max(dot(normal, lightDir), 0.0);
    float shadowVisibility = CalculateShadowVisibility(input.lightSpacePosition, normal, lightDir, textureFlags);
    ...
    {
        emissive *= SampleBindless(emissiveTextureIndex, input.uv).rgb;
    }
